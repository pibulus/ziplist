...
export default class ListRoom implements Party.Server {
  ...
  async onConnect(conn: Party.Connection, ctx: Party.ConnectionContext) {
    ...
    console.log(
      `[ListRoom ${this.party.id}] User ${avatar} joined (${Object.keys(presence).length} online)`,
    );
  }
  ...
  async onClose(conn: Party.Connection) {
    ...
    console.log(
      `[ListRoom ${this.party.id}] User ${user?.avatar || conn.id} left (${Object.keys(presence).length} online)`,
    );
  }
  ...
  async onMessage(message: string, sender: Party.Connection) {
    ...
    // Broadcast the enriched message to all connected users (except sender)
    this.party.broadcast(JSON.stringify(msg), [sender.id]);

    console.log(
      `[ListRoom ${this.party.id}] ${senderUser?.avatar || "Unknown"} sent ${msg.type}`,
    );
  }
  ...
  private async getPresence(): Promise<Record<string, PresenceUser>> {
    return (
      (await this.party.storage.get<Record<string, PresenceUser>>(
        "presence",
      )) || {}
    );
  }
