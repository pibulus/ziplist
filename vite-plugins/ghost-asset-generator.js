...
async function generateAllGhostAssets() {
  ...
  for (const theme of THEMES) {
    ...
    saveSvg(
      path.join(ASSETS_DIR, `talktype-icon-bg-gradient${theme === 'peach' ? '' : '-' + theme}.svg`),
      backgroundGhost
    );
    
    // Also save at root level (matching existing structure)
    saveSvg(
      path.join(STATIC_DIR, `talktype-icon-bg-gradient${theme === 'peach' ? '' : '-' + theme}.svg`),
      backgroundGhost
    );
  ...
  saveSvg(path.join(STATIC_DIR, 'ghost-template.svg'), generateStaticGhostSvg('peach'));
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" width="1024" height="1024">
  ...
  await generatePng(combinedSvg, path.join(STATIC_DIR, 'favicon-light.png'), 32);
  await generatePng(combinedSvg, path.join(STATIC_DIR, 'favicon-dark.png'), 32);
  await generatePng(combinedSvg, path.join(STATIC_DIR, 'apple-touch-icon.png'), 180);
  ...
  await generatePng(combinedSvg, path.join(ICONS_DIR, 'icon-maskable-512x512.png'), 512);
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" width="1024" height="1024">
  ...
  await generatePng(lightThemeSvg, path.join(ICONS_DIR, 'icon-maskable-light-512x512.png'), 512);
  await generatePng(darkThemeSvg, path.join(ICONS_DIR, 'icon-maskable-dark-512x512.png'), 512);
