...
import { listToText, textToItems } from "../src/lib/services/lists/listTextFormat.js";
...
// -----------------------------------------------------------------------------
console.log("🔥 [TEST 1] Parser & Tag Engine Throughput (10,000 operations)...");
...
for (let i = 0; i < 5000; i++) {
  ...
  if (i === 0) {
    assert(items.length >= 6, `Parsed ${items.length} items with checkboxes and tags`);
    assert(serialized.includes("- [ ] 2x Organic Honeycrisp apples"), "Serialized preserved markdown checkboxes");
  }
}
const parseDuration = (performance.now() - parseStart).toFixed(2);
console.log(`  ⚡ 5,000 text-to-item + serialize cycles completed in ${parseDuration}ms (${(5000 / (parseDuration / 1000)).toFixed(0)} ops/sec)`);

// Tag extraction & coherence stress test
const tagStart = performance.now();
const vocabulary = ["groceries", "fruit", "tools", "hardware", "diy", "music", "guitars"];
for (let i = 0; i < 5000; i++) {
  const extracted = extractTags("Buy 5 fresh #apples and #tomatoes for #cooking at the #supermarket", vocabulary);
  const consolidated = cohereTag("grocery", vocabulary);
  const consolidatedGuitars = cohereTag("guitar", vocabulary);
  if (i === 0) {
    assert(extracted.tags.length >= 4, `Extracted ${extracted.tags.length} distinct tags cleanly`);
    assert(consolidated === "groceries", "Tag cohere: 'grocery' snapped to canonical 'groceries'");
    assert(consolidatedGuitars === "guitars", "Tag cohere: 'guitar' snapped to canonical 'guitars'");
  }
}
const tagDuration = (performance.now() - tagStart).toFixed(2);
console.log(`  ⚡ 5,000 tag extraction & coherence operations completed in ${tagDuration}ms (${(5000 / (tagDuration / 1000)).toFixed(0)} ops/sec)`);
...
const validToken = makeToken({ sub: "contributor_user", exp: Date.now() + 100000 }, SECRET);
assert(verifyToken(validToken, SECRET).valid === true, "Valid issued token passes HMAC verification");
...
assert(rejectedForgedTokens === 1000, `1,000/1,000 forged signature attacks rejected (100% protection)`);
...
for (let i = 0; i < NUM_WRITES; i++) {
  concurrencyPromises.push(
    withStoreLock(STORE_NAME, async () => {
      const data = (await testStorage.get(STORE_NAME)) || { count: 0, items: [] };
      data.count += 1;
...
assert(finalStoreData.count === NUM_WRITES, `500 concurrent mutex locked writes completed without race condition (count: ${finalStoreData.count}/${NUM_WRITES})`);
assert(finalStoreData.items.length === NUM_WRITES, `500 items stored with zero data loss or collisions`);
...
try {
  ...
  for (let i = 0; i < 15; i++) {
    burstRequests.push(
      fetch(`${PROD_URL}/api/contributor/redeem`, {
        ...
        body: JSON.stringify({ code: i === 0 ? "ZL-PABLO-DEV" : `ZL-INVALID-CODE-${i}` })
      }).then(async (res) => {
  ...
  assert(validRedeem.status === 200 && validRedeem.body.valid === true, `Founder code ZL-PABLO-DEV verified live on ${PROD_URL} (Status 200)`);
  
  const invalidRejections = results.slice(1).filter(r => r.status === 401 || r.status === 400 || (r.status === 200 && r.body.valid === false));
  assert(invalidRejections.length === 14, `14/14 bogus codes gracefully rejected with 401 Unauthorized`);
  
  console.log(`  ⚡ 15 parallel production HTTP requests completed in ${burstDuration}ms (Avg ${(burstDuration / 15).toFixed(1)}ms / req)`);
} catch (e) {
...
// -----------------------------------------------------------------------------
console.log("\n============================================================");
console.log(`🏁 STRESS TEST COMPLETE: ${totalPassed} PASSED / ${totalFailed} FAILED`);
console.log("============================================================\n");
