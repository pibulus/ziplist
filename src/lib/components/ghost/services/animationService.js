...
export function applyInitialLoadEffect(ghostSvg) {
  ...
  console.log(
    "[applyInitialLoadEffect] Applying combined grow-and-wobble effect...",
  );
  ...
  if (!wobbleGroup) {
    console.error(
      "[applyInitialLoadEffect] Could not find .ghost-wobble-group element.",
    );
    return;
  }
  ...
  requestAnimationFrame(() => {
    ...
    if (!document.body.contains(wobbleGroup)) {
      console.warn(
        "[applyInitialLoadEffect] Wobble group removed before animation could start.",
      );
      return;
    }
    ...
    console.log(
      `[applyInitialLoadEffect] Scheduling cleanup timeout for ${cleanupDelay}ms`,
    );
