...
function createGhostStateStore() {
  ...
  function setAnimationState(newState) {
    ...
    if (
      currentState.current === ANIMATION_STATES.INITIAL &&
      (newState === ANIMATION_STATES.IDLE ||
        newState === ANIMATION_STATES.RECORDING)
    ) {
      debugLog(
      ...
      if (currentState.eyesClosed) {
        debugLog(
          `Eyes were closed, forcing them open during transition from INITIAL state`,
        );
    ...
    if (isRecording && !wasRecording) {
      ...
      if (wobbleGroupStart) {
        ...
        _state.update((s) => ({
          ...
          stateTimeouts: {
            ...s.stateTimeouts,
            startWobbleCleanup: startTimeoutId,
          },
        }));
      } else {
        debugLog(
    ...
    // --- Recording Stop Sequence ---
      ...
      if (wobbleGroupStop) {
        ...
        _state.update((s) => ({
          ...
          stateTimeouts: {
            ...s.stateTimeouts,
            stopWobbleCleanup: stopTimeoutId,
          },
        }));
      } else {
        debugLog(
  ...
  function setEyePosition(x, y) {
    ...
    if (isNaN(x) || isNaN(y)) {
      debugLog(
        `Received NaN eye position, using fallback. Original: [${x}, ${y}], Safe: [${safeX}, ${safeY}]`,
        "warn",
      );
    }
  ...
  function triggerReaction() {
    ...
    if (
      currentStateVal === ANIMATION_STATES.IDLE ||
      currentStateVal === ANIMATION_STATES.RECORDING
    ) {
