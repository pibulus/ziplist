<script>
  ...
  onDestroy(() => {
    ...
    // Debug log
    if (debug)
      console.log("Ghost component destroyed and all animations cleaned up");
  });
  ...
  $: if (
    ...
    setTimeout(() => {
      ...
      if (debug)
        console.log("[Ghost.svelte] Triggering post-wake-up double blink.");
  ...
  $: if (
    $ghostStateStore.current !== ANIMATION_STATES.IDLE &&
    wakeUpBlinkTriggered
  ) {
    if (debug)
      console.log("[Ghost.svelte] Resetting wakeUpBlinkTriggered flag.");
  ...
  $: if (
    browser &&
    (isRecording !== lastRecordingState || isProcessing !== lastProcessingState)
  ) {
...
<button
  ...
  class="ghost-container theme-{currentTheme} 
    ...
    ? CSS_CLASSES.SPIN
    : ''}
      {$ghostStateStore.current === ANIMATION_STATES.ASLEEP
    ? CSS_CLASSES.ASLEEP
    : ''}
      {$ghostStateStore.current === ANIMATION_STATES.WAKING_UP
    ? CSS_CLASSES.WAKING_UP
    : ''}
...
>
  ...
  >
    <defs>
      ...
      <linearGradient id="nocturneGradient" x1="0%" y1="0%" x2="100%" y2="100%">
...
<style>
  ...
  :global(.initial-load) .ghost-layer {
    animation: grow-ghost var(--ghost-grow-duration, 2s)
  ...
  .ghost-svg.theme-focus #ghost-shape {
    animation-duration:
      var(--ghost-shimmer-duration, 5s), var(--ghost-focus-flow-duration, 9s);
    animation-timing-function:
      var(--ghost-shimmer-ease, ease-in-out),
      var(--ghost-focus-flow-ease, cubic-bezier(0.4, 0, 0.6, 1));
  }

  .ghost-svg.theme-chill #ghost-shape {
    animation-duration:
      var(--ghost-shimmer-duration, 6s), var(--ghost-chill-flow-duration, 10s);
    animation-timing-function:
      var(--ghost-shimmer-ease, ease-in-out),
      var(--ghost-chill-flow-ease, cubic-bezier(0.4, 0, 0.6, 1));
  }

  .ghost-svg.theme-zen #ghost-shape {
    animation-duration:
      var(--ghost-shimmer-duration, 7s), var(--ghost-zen-flow-duration, 12s);
    animation-timing-function:
      var(--ghost-shimmer-ease, ease-in-out),
      var(--ghost-zen-flow-ease, cubic-bezier(0.4, 0, 0.6, 1));
  }

  .ghost-svg.theme-nocturne #ghost-shape {
    animation-duration: var(--ghost-nocturne-flow-duration, 9s);
    animation-timing-function: var(
      --ghost-nocturne-flow-ease,
  ...
  .ghost-svg.debug-animation #ghost-shape {
    animation-duration:
      calc(var(--ghost-shimmer-duration, 5s) * 2),
      calc(var(--ghost-focus-flow-duration, 9s) * 2) !important;
  }
</style>
