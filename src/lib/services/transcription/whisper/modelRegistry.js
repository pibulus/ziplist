 ...
 * Manages model configuration for ZipList's transcription needs
 */
...
export const selectedModel = derived(
  ...
  ([$modelRegistry, $userPreferences]) => {
    ...
    return (
      $modelRegistry.models.find((model) => model.id === modelId) ||
      $modelRegistry.models[0]
    );
  },
);
...
export function getModelInfo(modelId = "tiny") {
  ...
  const model =
    registry.models.find((m) => m.id === modelId) || registry.models[0];

  if (!model) {
    console.warn(
      `Model ${modelId} not found in registry, using first available model`,
    );
