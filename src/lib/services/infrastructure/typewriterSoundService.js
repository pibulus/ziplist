...
export class TypewriterSoundService {
  ...
  isLoaded(pack = this.pack) {
    return Boolean(
      this.audioBuffer && this.configData && this.loadedPack === pack,
    );
  }
  ...
  getFetch() {
    return (
      this.fetchImpl ||
      this.host?.fetch?.bind(this.host) ||
  ...
  async loadPack(packName) {
    ...
    try {
      ...
      if (!isResponseOk(configResponse) || !isResponseOk(audioResponse))
        return false;
  ...
  getKeyFromInputEvent(event) {
    ...
    if (INPUT_TYPE_KEYS[event.inputType])
      return INPUT_TYPE_KEYS[event.inputType];
  ...
  async playKey(key, options = {}) {
    ...
    const soundData =
      this.configData.defines?.[keyId] || this.configData.defines?.["65"];
