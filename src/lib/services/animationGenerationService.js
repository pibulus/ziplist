    ...
    try {
      ...
      if (!initialized) {
        try {
          ...
          console.log(
            "⚠️ Preloading failed, continuing with animation generation:",
            err,
          );
        }
      }
      ...
      const prompt = promptManager.getPrompt("generateAnimation", {
        description,
      });
...
.${animationData.name} {
  ...
  applyAnimation(ghostSvg, animationData) {
    ...
    if (!targetElement) {
      throw new Error(
        `Target element '${animationData.target}' not found in ghost SVG`,
      );
    }
