...
function fromBase64Url(encoded) {
  let normalized = encoded.replace(/-/g, "+").replace(/_/g, "/").replace(/ /g, "+");
...
for (const text of [
  ...
  "寿司 مرحبا шар",
  JSON.stringify({ name: "Café — rün", items: [{ text: "→ x", checked: true }] }),
]) {
  assert.equal(fromBase64Url(toBase64Url(text)), text, `round-trip failed: ${text}`);
}

// base64url never emits the characters that a URL fragment mangles.
for (let i = 0; i < 400; i++) {
  const encoded = toBase64Url("x".repeat(i % 7) + String.fromCharCode(0xfb, 0xef, 0xbe, i % 256));
  assert.ok(!/[+/=]/.test(encoded), `base64url leaked a URL-unsafe char: ${encoded}`);
  assert.equal(
...
assert.equal(fromBase64Url(withPlus.replace(/\+/g, " ")), fromBase64Url(withPlus));
